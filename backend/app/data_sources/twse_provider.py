...
class TwseProvider(MarketDataProvider):
    provider_name = "twse-official"
    data_source = "Official"
    is_realtime = False
    delay_minutes = None
    license_note = "TWSE 官方公開資料，可能為盤後或延遲資料。"
    ...
    async def get_latest_quote(self, symbol: str) -> QuoteData | None:
        raise ProviderUnavailable("TWSE 官方 provider 第一版僅作來源狀態與未來 adapter，不假裝即時報價。")
    ...
    async def get_kline(self, symbol: str, interval: Interval, start: date, end: date) -> list[PriceBar]:
        if not self.supports_interval(interval):
            raise UnsupportedInterval("TWSE 官方公開資料目前不支援此週期。")
        raise ProviderUnavailable("TWSE 日 K endpoint 會依官方 OpenAPI 調整；目前使用 fallback。")
    ...
    async def health_check(self) -> SourceHealth:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                response = await client.get(f"{self.settings.twse_openapi_base_url}/exchangeReport/STOCK_DAY_AVG_ALL")
            return SourceHealth(
                provider=self.provider_name,
                datasetType="official_market_data",
                status="ok" if response.status_code < 500 else "degraded",
                recordsFetched=1 if response.status_code < 500 else 0,
                errorMessage=None if response.status_code < 500 else f"TWSE HTTP {response.status_code}",
                supportsLatestQuote=False,
                supportsIntraday=False,
                supportsDaily=True,
                isRealtime=False,
            ...
            return SourceHealth(
                ...
                datasetType="official_market_data",
                status="error",
                errorMessage=f"TWSE 官方資料源連線失敗：{exc}",
                supportsLatestQuote=False,
                supportsIntraday=False,
                supportsDaily=True,
                isRealtime=False,
